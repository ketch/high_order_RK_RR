...
C --- BASIC INTEGRATION STEP  
  30   CONTINUE
       ...
       KC=K-1
       num_loops = ceiling((2*K+2)/4.d0)
C       write(*,*) num_loops
!$OMP PARALLEL DEFAULT(SHARED) PRIVATE(J,jj) FIRSTPRIVATE(Y,DY,YH1,YH2) 
!$OMP DO SCHEDULE(DYNAMIC)
       DO 40 J=1,num_loops
           if (J.eq.1) then ! do the last one
     ...
     3         IOUT,IPT,YSAFE,ICOMP,NRD,RPAR,IPAR,NFCN)
           elseif ((K/2.d0.eq.K/2).and.(J.eq.num_loops)) then
           CALL MIDEX(K/2,X,Y,H,HMAX,N,FCN,DY,YH1,YH2,DZ,T,NJ,HH,W,
     1         ERR,FAC,A,SAFE1,UROUND,FAC1,FAC2,SAFE2,SCAL,ATOV,SAFE3,
     2         REJECT,KM,RTOL,ATOL,ITOL,MSTAB,JSTAB,ERROLD,FSAFE,LFSAFE,
     3         IOUT,IPT,YSAFE,ICOMP,NRD,RPAR,IPAR,NFCN)
           else                
           jj = (J-1)
           CALL MIDEX(jj,X,Y,H,HMAX,N,FCN,DY,YH1,YH2,DZ,T,NJ,HH,W,
     1         ERR,FAC,A,SAFE1,UROUND,FAC1,FAC2,SAFE2,SCAL,ATOV,SAFE3,
     2         REJECT,KM,RTOL,ATOL,ITOL,MSTAB,JSTAB,ERROLD,FSAFE,LFSAFE,
     3         IOUT,IPT,YSAFE,ICOMP,NRD,RPAR,IPAR,NFCN)
           CALL MIDEX(K-jj,X,Y,H,HMAX,N,FCN,DY,YH1,YH2,DZ,T,NJ,HH,W,
     1         ERR,FAC,A,SAFE1,UROUND,FAC1,FAC2,SAFE2,SCAL,ATOV,SAFE3,
     2         REJECT,KM,RTOL,ATOL,ITOL,MSTAB,JSTAB,ERROLD,FSAFE,LFSAFE,
     3         IOUT,IPT,YSAFE,ICOMP,NRD,RPAR,IPAR,NFCN)
           endif
...
C --- SCALING
  ...
  67   ERR=ERR+((T(1,I)-T(2,I))/SCAL(I))**2 
       ERR=SQRT(ERR/N)
       IF ((ERR*UROUND.GE.1.D0) .OR.  (J.GT.2.AND.ERR.GE.ERROLD)) THEN
           ATOV=.TRUE.
           H=H*SAFE3
           REJECT=.TRUE.
       END IF
...
C       CALL FCN(N,X+H,YH2,DY,RPAR,IPAR) 
C         IF (IOUT.GE.2.AND.NJMID.LE.2*J-1) THEN 
C            IPT=IPT+1
C            DO 39 I=1,NRD 
C39          FSAFE(IPT,I)=DY(ICOMP(I)) 
C         END IF
       DO 40 I=1,N
  40   T(J,I)=YH2(I)
C 40   T(J,I)=(YH1(I)+YH2(I)+HJ*DY(I))/2.D0
       NFCN=NFCN+NJ(J)  
       RETURN
  79   ATOV=.TRUE.
       H=H*SAFE3
